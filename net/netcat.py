...
def run_shell(cmd: str) -> bytes:
    ...
    try:
        ...
        return e.output or f"[!] error : retrun_code={e.returncode}\r\n".encode()
...
def _handle_upload(sock: socket.socket, destination: str) -> None:
    ...
    try:
        with open(destination, "wb") as f:
            f.write(buffer)
        msg = f"[ok] {len(buffer)} bytes writen to {destination}\r\n"
        print(msg.strip())
        sock.sendall(msg.encode())
    except OSError as e:
        msg = f"[!] error : cannot writen {destination} : {e}\r\n"
        print(msg.strip())
        sock.sendall(msg.encode())
...
def server_mode(cfg: Config) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as srv:
        ...
        print(f"[*] Server listen on {cfg.target}:{cfg.port}")
...
def client_mode(cfg: Config, initial_buffer: str = "") -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        try:
            ...
            print(f"[*] Connect to {cfg.target}:{cfg.port}")
        except ConnectionRefusedError:
            print(f"[!] Connexion refuse : {cfg.target}:{cfg.port}")
            sys.exit(1)
